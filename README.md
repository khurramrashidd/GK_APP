...
## Keeping dependencies current (#23)

I deliberately have NOT run `flutter pub upgrade --major-versions` for you.
Your `pubspec.lock` currently sits on major versions behind for several
packages, and those specific jumps are breaking changes:

- `flutter_riverpod` 2.x -> 3.x  (provider API changes)
- `cloud_firestore` 5.x -> 6.x, `firebase_auth` 5.x -> 6.x
- `google_sign_in` 6.x -> 7.x  (sign-in flow was reworked)

Each of those would require code changes across many files, and I have no
way to compile or run the app here to verify the result — so bumping them
blind would very likely hand you a broken build.

Do it yourself, one at a time, when you have time to test:

```bash
flutter pub outdated                 # see what's behind
flutter pub upgrade                  # safe: within existing constraints
flutter pub upgrade --major-versions # RISKY: do one package at a time
```

Patch/minor upgrades (`flutter pub upgrade`) are safe to run any time and
worth doing before each release build.

## Extending a battle mid-session

After a match finishes, either player can tap **"Add more questions"** and
propose 5, 10 or 15 more. The other player sees the offer on their result
screen and accepts or declines. On accept, both players are returned to the
match automatically with fresh questions appended — scores so far are kept,
so it continues rather than restarting.

Two details that matter:

- **Accept runs in a transaction.** Appending the new question ids and
  clearing both finished-flags must happen together. As separate writes, a
  failure between them would leave players on different question lists, or
  one stuck on the result screen while the other played on.
- **Questions already used in the match are excluded**, so an extension never
  repeats what was just answered. If the subject has none left, the proposal
  is cleared with a message rather than failing silently.

## Following domains

Tap the heart on a domain card to follow it. Followed domains sort to the top
of the home grid. Stored as a list on the user's own profile — one write, no
extra collection, no extra reads.

**Live "active users" was removed on purpose.** It needed a presence document
per user plus repeated aggregation queries, and without a Cloud Function
(paid tier) to clean up stale entries it could not be made both accurate and
free-tier safe. Not worth the read budget.
