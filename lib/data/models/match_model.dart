...
class MatchModel {
  ...
  /// Rematch-extension proposal, set when one player asks to continue the
  /// match with more questions after finishing.
  ///
  /// null when nobody has proposed. Cleared on accept or decline, so a stale
  /// proposal can never linger and re-trigger the dialog.
  final String? extendProposedByUid;
  final String? extendProposedByName;
  ...
  MatchModel({
    ...
    this.extendProposedByUid,
    this.extendProposedByName,
    this.extendCount,
  ...
  /// True when someone has asked to extend and it hasn't been answered yet.
  bool get hasExtendProposal => extendProposedByUid != null;

  /// True when the proposal is waiting on THIS user (i.e. they didn't make it).
  bool waitingOnMe(String myUid) =>
      hasExtendProposal && extendProposedByUid != myUid;
  ...
  factory MatchModel.fromMap(String id, Map<String, dynamic> m) {
    ...
    return MatchModel(
      ...
      extendProposedByUid: m['extendProposedByUid'] as String?,
      extendProposedByName: m['extendProposedByName'] as String?,
      extendCount: (m['extendCount'] as num?)?.toInt(),
