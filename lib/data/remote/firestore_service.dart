...
class FirestoreService {
  ...
  Future<void> saveQuizResult(String uid, QuizResult result,
      String displayName, int newTotalScore,
      {int currentStreak = 0}) async {
    final batch = _db.batch();
    final histRef = _db
        .collection('users')
        .doc(uid)
    ...
    // Lifetime questions-answered counter, incremented atomically alongside
    // the history write so the two can never disagree.
    batch.set(
      _db.collection('users').doc(uid),
      {
        'questionsAnswered': FieldValue.increment(result.total),
        'correctAnswers': FieldValue.increment(result.correct),
      },
      SetOptions(merge: true),
    );
    ...
    // Public leaderboard slice. Still deliberately minimal — display name
    // and play statistics only, never email, location or anything personal.
    // The counters are incremented here rather than copied from the profile
    ...
    batch.set(
        lbRef,
        {
          ...
          'questionsAnswered': FieldValue.increment(result.total),
          'correctAnswers': FieldValue.increment(result.correct),
          // Set (not incremented) — a streak is a current value, not a total.
    ...
    return _db.collection('leaderboard').snapshots().map((snap) {
      final list = snap.docs.map((d) {
        ...
        return <String, dynamic>{
          'uid': d.id,
          'displayName': (m['displayName'] ?? 'Anonymous') as String,
      ...
      // Default order is by score; the accuracy and streak boards re-sort
      // this same list client-side, so one stream feeds all three.
      list.sort((a, b) =>
  ...
  /// Grants/revokes ad-free premium. Manual for now (testers, supporters);
  /// a billing integration would call this same method after a purchase.
  ...
  // ------------------------------- Usernames ----------------------------------
  // Uniqueness is enforced by a dedicated `usernames` collection, one doc per
  // lowercase handle holding the owning uid. A transaction claims the new doc
  // and releases the old one together, so two people racing for the same
  // handle can't both win — checking "is it taken?" then writing separately
  // would leave exactly that gap.

  /// Claims [username] for [uid], releasing [previous] if given.
  /// Throws a StateError with a user-readable message if already taken.
  Future<void> claimUsername({
    ...
    await _db.runTransaction((tx) async {
      ...
      // Store the display name here too. Search then reads ONLY this
      // collection — a regular user has no permission to read other
      // people's /users documents, and shouldn't: a handle and a display
      // name is all that finding someone requires.
      tx.set(newRef, {
        'uid': uid,
        'username': username,
        'usernameLower': lower,
      ...
      if (previous != null && previous.toLowerCase() != lower) {
        tx.delete(_db.collection('usernames').doc(previous.toLowerCase()));
      }
  ...
  /// Finds players by username OR real name, as a prefix match.
  ///
  /// Two queries, merged:
  ///
  ///  * BY USERNAME — matched on the DOCUMENT ID, because the id of a
  ///    usernames document *is* the lowercase handle. This matters: handles
  ///    claimed before the `usernameLower` field existed don't have that
  ///    field, so a query filtering on it silently matches nothing. The
  ///    document id is always there.
  ///
  ///  * BY NAME — matched on `displayNameLower`, which only exists on
  ///    handles saved since that field was added. Admin > Rebuild username
  ///    index backfills it for older ones.
  ///
  /// Firestore has no OR across different fields, hence two round trips;
  /// callers should debounce so typing doesn't fire one per keystroke.
  Future<List<({String uid, String username, String displayName})>>
      searchUsersByUsername(String query, {int limit = 10}) async {
    final q = query.trim().toLowerCase();
    if (q.isEmpty) return [];

    // \uf8ff is the conventional high-codepoint sentinel meaning
    // "anything that sorts after this prefix".
    final end = '$q\uf8ff';
    final found = <String, ({String uid, String username, String displayName})>{};
    ...
    void absorb(QuerySnapshot<Map<String, dynamic>> snap) {
      for (final d in snap.docs) {
        ...
        found[uid] = (
          uid: uid,
          username: (m['username'] ?? d.id) as String,
    ...
    try {
      ...
      // Ignore and still try the name query — a partial result beats none.
    }
    ...
    try {
      ...
      // displayNameLower may not exist on older documents.
    }

    return found.values.take(limit).toList();
  }

  /// Backfills `usernameLower` and `displayNameLower` onto username
  /// documents created before those fields existed, so name search finds
  /// people who claimed a handle earlier. Admin-only; safe to re-run.
  Future<int> rebuildUsernameIndex() async {
  ...
  // -------------------------- Reset (self and season) --------------------------

  /// A user clearing THEIR OWN statistics. Touches only their own documents,
  /// so it needs no admin rights and no rules change.
  Future<int> resetMyStats(String uid) async {
    ...
    final hist =
        await _db.collection('users').doc(uid).collection('history').get();
    var deleted = 0;
    var batch = _db.batch();
    var inBatch = 0;
  ...
  /// Wipes every competitive statistic and all quiz history, app-wide.
  /// IRREVERSIBLE — history is deleted, not archived.
  Future<({int users, int historyDeleted})> resetAllStats({
    void Function(String message)? onProgress,
  }) async {
    onProgress?.call('Reading users...');
    late final QuerySnapshot<Map<String, dynamic>> users;
    try {
      users = await _db.collection('users').get();
    } catch (e) {
      throw StateError('Could not read the user list.\n\n'
    ...
    for (final userDoc in users.docs) {
      try {
        ...
        throw StateError('Could not reset user ${userDoc.id}.\n\n'
            'The rules must let an admin update these counters on another '
            'user. Deploy the latest rules:\n'
            '  firebase deploy --only firestore:rules\n\n'
            'Original error: $e');
      }

      // NOTE: listing history is a SEPARATE permission from deleting it.
      // Firestore has no "delete everything matching" call — you enumerate,
      // then delete — so the rules must allow an admin to list history.
      late final QuerySnapshot<Map<String, dynamic>> hist;
      try {
        hist = await userDoc.reference.collection('history').get();
      } catch (e) {
        throw StateError('Could not read the quiz history of user '
            '${userDoc.id}.\n\n'
            'Deleting history needs LIST permission as well as delete. '
            'Deploy the latest rules:\n'
            '  firebase deploy --only firestore:rules\n\n'
            'Original error: $e');
      }

      var hBatch = _db.batch();
      var inHBatch = 0;
      for (final h in hist.docs) {
        hBatch.delete(h.reference);
        historyDeleted++;
        inHBatch++;
        if (inHBatch >= 400) {
          await hBatch.commit();
      ...
      if (inHBatch > 0) await hBatch.commit();

      processed++;
      if (processed % 5 == 0 || processed == users.docs.length) {
        onProgress?.call('Reset $processed of ${users.docs.length} users...');
      }
    }

    onProgress?.call('Clearing leaderboard...');
    final lb = await _db.collection('leaderboard').get();
    var batch = _db.batch();
    var inBatch = 0;
    for (final d in lb.docs) {
    ...
    return (users: users.docs.length, historyDeleted: historyDeleted);
  }

  // --------------------------------- Friends ----------------------------------
  // Friendship is stored twice — once under each user — so each person can
  // read their own list without permission to query everyone else's.
  // Requests live under the RECIPIENT, keyed by sender uid so the same person
  // can't queue duplicates.
  ...
  Future<void> sendFriendRequest({
    ...
    if (toUid == fromUid) throw StateError('You cannot add yourself.');
    final existing = await _db
        ...
        .doc(toUid)
        .get();
    if (existing.exists) throw StateError('You are already friends.');
  ...
  Stream<List<FriendModel>> streamFriends(String uid) {
    return _db
        ...
        .snapshots()
        .map((s) =>
            s.docs.map((d) => FriendModel.fromMap(d.id, d.data())).toList());
  }

  /// Writes the friendship into BOTH lists and removes the request in one
  /// batch, so a partial accept is impossible.
  Future<void> acceptFriendRequest({
    ...
    final batch = _db.batch();
    batch.set(
      _db.collection('users').doc(myUid).collection('friends').doc(request.fromUid),
      FriendModel(
        uid: request.fromUid,
    ...
    batch.set(
      _db.collection('users').doc(request.fromUid).collection('friends').doc(myUid),
      FriendModel(uid: myUid, username: myUsername, displayName: myName).toMap(),
    );
    batch.delete(_db
        .collection('users')
        .doc(myUid)
  ...
  Future<void> declineFriendRequest(String myUid, String fromUid) async {
    await _db
        .collection('users')
        .doc(myUid)
        .collection('friend_requests')
  ...
  Future<void> removeFriend(String myUid, String friendUid) async {
    final batch = _db.batch();
    batch.delete(
        _db.collection('users').doc(myUid).collection('friends').doc(friendUid));
    batch.delete(
        _db.collection('users').doc(friendUid).collection('friends').doc(myUid));
    await batch.commit();
  }

  /// Refreshes cached activity figures on my copy of each friend.
  /// One read per friend, so it runs on demand (pull-to-refresh).
  Future<void> refreshFriendActivity(
      String myUid, List<FriendModel> friends) async {
    if (friends.isEmpty) return;
    final batch = _db.batch();
    for (final f in friends) {
      final snap = await _db.collection('users').doc(f.uid).get();
      if (!snap.exists) continue;
      final d = snap.data()!;
      batch.update(
        _db.collection('users').doc(myUid).collection('friends').doc(f.uid),
        {
          'displayName': d['name'] ?? f.displayName,
          'username': d['username'] ?? f.username,
          'totalScore': (d['totalScore'] as num?)?.toInt() ?? 0,
          'questionsAnswered': (d['questionsAnswered'] as num?)?.toInt() ?? 0,
  ...
  /// Bulk operations for the admin log screen. Batched so a selection of
  /// 100 logs is one network round-trip rather than 100.
  Future<void> setErrorLogsResolved(List<String> ids, bool resolved) async {
    if (ids.isEmpty) return;
    var batch = _db.batch();
    var n = 0;
    for (final id in ids) {
      batch.update(
          _db.collection('error_logs').doc(id), {'resolved': resolved});
      n++;
      if (n >= 400) {
        await batch.commit();
        batch = _db.batch();
  ...
  Future<void> deleteErrorLogs(List<String> ids) async {
    if (ids.isEmpty) return;
    var batch = _db.batch();
    var n = 0;
    for (final id in ids) {
      batch.delete(_db.collection('error_logs').doc(id));
      n++;
      if (n >= 400) {
        await batch.commit();
        batch = _db.batch();
