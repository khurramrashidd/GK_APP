...
final contentUpdatesProvider =
  ...
  return ref.watch(firestoreServiceProvider).streamContentUpdates();
});
...
final unseenUpdateCountProvider = Provider<int>((ref) {
  ...
  if (since == null) {
    // Never opened the feed: only count recent items, so a brand-new user
    // isn't greeted with a badge for the app's entire history.
    final cutoff = DateTime.now().subtract(const Duration(days: 14));
    return updates
        .where((u) => (u.createdAt ?? cutoff).isAfter(cutoff))
        .length;
  }
  return updates
...
/// Users can always reach Reels from the drawer regardless of this.
