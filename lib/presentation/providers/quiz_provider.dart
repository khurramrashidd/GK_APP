...
class QuizNotifier extends StateNotifier<QuizState> {
  ...
  Future<void> startQuiz(
    ...
    try {
      ...
      // A preloaded pool has ALREADY been sized by the caller (the custom /
      // random quiz screen honours the count the user picked), so trimming
      // it again to questionsPerQuiz silently capped every custom quiz at
      // 10 — asking for 20 gave 10. Only apply the default cap when we
      // fetched the pool ourselves.
      final picked = preloaded != null
          ? shuffled
          : shuffled.take(AppConstants.questionsPerQuiz).toList();
