...
class _AdminDomainsScreenState extends ConsumerState<AdminDomainsScreen> {
  ...
  /// Whether counts are shown. Lives in a persisted provider
  /// (showQuestionCountsProvider) so the choice survives app restarts —
  /// previously this was local state and reset to off every time the screen
  /// was opened.
  bool get _showCounts => ref.watch(showQuestionCountsProvider);
  ...
  Future<void> _createDomain() async {
    ...
    // Deliberately NOT announced here: a new domain starts hidden and has no
    // questions. It's announced when you unhide it.
    _snack('Domain "$name" created. It is hidden until you show it.');
  }
  ...
  Future<void> _toggleDomainActive(DomainModel d) async {
    ...
    final nowVisible = !d.isActive;
    await _persist(d.copyWith(isActive: nowVisible));
    if (nowVisible) {
      await _announce('domain', 'New category: ${d.name}', domainId: d.id);
    }
  }
  ...
  Future<void> _announce(String kind, String title,
      ...
      String? domainId,
      String? subjectId,
      bool isVisible = true}) async {
    // Never announce content users can't actually see. New domains and
    // subjects are created hidden, so announcing on creation told everyone
    // about something that wasn't there yet. Announce when it's UNHIDDEN
    // instead.
    if (!isVisible) return;
    try {
  ...
  Future<void> _addSubject(DomainModel d) async {
    ...
    // Not announced on creation — see _announce. Announced when unhidden.
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: !_showCounts
                ? const Text('Question counts hidden — tap to show',
          ...
          Switch(
            value: _showCounts,
            onChanged: (v) =>
                ref.read(showQuestionCountsProvider.notifier).set(v),
          ),
        ],
      ),
    );
