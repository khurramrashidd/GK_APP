...
class _AdminUploadScreenState extends ConsumerState<AdminUploadScreen> {
  ...
  /// Sample shown by "Insert sample". Matches the CURRENT question format,
  /// including classLevel. Note the answer isn't first here on purpose —
  /// answer positions are shuffled automatically on upload anyway, so the
  /// index you supply is just "which of these is right", not a position you
  /// need to vary yourself.
  static const _sample = '''[
  {
    "question": "Which river is the longest in India?",
    "options": ["Godavari", "Ganga", "Yamuna", "Narmada"],
    "correctOptionIndex": 1,
    "explanation": "The Ganga is the longest river flowing within India, at about 2,525 km.",
    "difficulty": 1,
    "classLevel": 6,
    "tags": ["india", "geography", "rivers"]
  },
  {
    "question": "The Tropic of Cancer passes through how many Indian states?",
    "options": ["Six", "Eight", "Five", "Ten"],
    "correctOptionIndex": 1,
  ...
  Map<String, dynamic> _buildDoc(
    ...
    // Randomise which position holds the correct answer. Question sets are
    // almost always authored with the answer first; without this, users can
    // score ~100% by always tapping option A.
    final rawIdx = raw['correctOptionIndex'];
    final shuffled = AnswerShuffler.shuffle(
      List<String>.from(raw['options'] ?? const []),
      rawIdx is int ? rawIdx : 0,
    );
    final opts = shuffled.options;
    ...
    return {
      ...
      // Optional school-class tag (6-12), used by the class filter. Kept
      // nullable: most questions don't need one.
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      _clearPickedFile();
      // Tell users new questions landed. Best-effort — a failed announcement
      // must never make a successful upload look like it failed.
      // Only tell users about questions they can actually reach. Uploading
      // into a hidden domain or subject used to notify everyone about
      // content that wasn't visible to them.
      final reachable = _domain!.isActive && _subject!.isActive;
      if (written > 0 && reachable) {
        try {
          await fs.postContentUpdate(ContentUpdateModel(
            id: '',
            kind: 'questions',
            title: '$written new question${written == 1 ? '' : 's'} added',
            detail: '${_domain!.name} > ${_subject!.name}',
            domainId: _domain!.id,
            subjectId: _subject!.id,
          ));
        } catch (_) {}
