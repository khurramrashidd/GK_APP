...
class _AdminUsersScreenState extends ConsumerState<AdminUsersScreen> {
  ...
  /// Grants/revokes premium (ad-free) manually. Useful for testers and early
  /// supporters before any billing integration exists. Long-press a user.
  Future<void> _togglePremium(UserProfile u) async {
    final granting = !u.isPremium;
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(granting ? 'Grant premium?' : 'Remove premium?'),
        content: Text(granting
            ? '${u.email} will not see ads once advertising is enabled.'
            : '${u.email} will see ads again once advertising is enabled.'),
        actions: [
          TextButton(
          ...
          FilledButton(
              ...
              child: Text(granting ? 'Grant' : 'Remove')),
        ],
      ),
    );
    if (ok != true) return;
