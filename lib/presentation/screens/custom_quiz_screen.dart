...
class _CustomQuizScreenState extends ConsumerState<CustomQuizScreen> {
  ...
  /// e.g. "12 subjects across 4 domains" — a compact summary of what a
  /// random/custom quiz drew from, derived from the questions themselves so
  /// it stays accurate however the pool was assembled.
  String _coverageLabel(List<QuestionModel> qs) {
    final subjects = qs.map((q) => q.subjectName).toSet();
    final domains = qs.map((q) => q.domainName).toSet();
    final sCount = subjects.length;
    final dCount = domains.length;
    if (sCount == 1) return subjects.first;
    return '$sCount subject${sCount == 1 ? '' : 's'} across '
        '$dCount domain${dCount == 1 ? '' : 's'}';
  }

  Future<void> _start(List<DomainModel> domains) async {
    setState(() => _busy = true);
    try {
      // Capture everything we need from `ref` BEFORE any awaits. Loading a
      // large pool takes seconds, and if the user backs out during it the
      // widget is disposed — touching `ref` afterwards throws
      ...
      // Bail out if the user left while questions were loading.
      if (!mounted) return;
      ...
      Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (_) => QuizScreen(
          // Record what was actually covered. Previously this stored
          // "Random" / "20 questions", so history couldn't tell you what you
          // had practised.
          domainName: _isRandom == true ? 'Random' : 'Custom',
          subjectName: _coverageLabel(picked),
        ),
      ));
    } finally {
