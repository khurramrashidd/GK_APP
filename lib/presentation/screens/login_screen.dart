...
class _LoginScreenState extends ConsumerState<LoginScreen>
  ...
  /// Signs in as a guest (anonymous). Full app access, but the account is
  /// tied to this device only — the app reminds them to upgrade.
  Future<void> _guest() async {
    setState(() => _loading = true);
    try {
      final cred = await ref.read(authServiceProvider).signInAsGuest();
      if (cred.user != null) await _afterAuth(cred.user!);
    } catch (e) {
      final msg = e.toString().contains('operation-not-allowed')
          ? 'Guest sign-in is not enabled yet. Enable Anonymous sign-in in '
              'Firebase Console > Authentication > Sign-in method.'
          : 'Could not continue as guest: $e';
      if (mounted) {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              // Try-before-you-sign-up. Uses Firebase anonymous auth, so the
              // guest gets a real uid and full app access; they're nudged to
              // upgrade later, which preserves their progress.
              TextButton.icon(
                style: TextButton.styleFrom(
                    minimumSize: const Size(double.infinity, 46)),
                icon: const Icon(Icons.person_outline_rounded),
                label: const Text('Skip for now — browse as guest'),
