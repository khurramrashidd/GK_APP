...
class QuizScreen extends ConsumerWidget {
  ...
  /// Seconds allowed per question. null = untimed (the default).
  ...
  void _skip(BuildContext context, WidgetRef ref, bool isLast,
    ...
    final msg = timedOut
        ? "Time's up! Trust your instinct next time — first guesses are often right."
        : _skipEncouragements[
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(_title),
          actions: [
            // Per-question countdown. The ValueKey resets it on each new
            // question; running out auto-skips, matching the Skip button.
            if (timerSeconds != null)
              QuestionTimer(
                key: ValueKey('timer_${q.id}'),
                seconds: timerSeconds!,
                paused: quiz.isCurrentRevealed,
                onTimeout: () => _skip(context, ref, quiz.isLast,
                    timedOut: true),
              ),
  ...
  Widget _revealPanel(BuildContext context, question, int selected) {
    ...
    return Card(
      color: (correct ? Colors.green : Colors.red).withValues(alpha: 0.10),
      child: Padding(
