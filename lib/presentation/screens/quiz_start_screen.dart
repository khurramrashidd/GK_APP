...
class _QuizStartScreenState extends ConsumerState<QuizStartScreen> {
  ...
  /// Seconds per question; null = untimed. Off by default so the existing
  /// relaxed experience is unchanged unless the user opts in.
  int? _timerSeconds;
  static const _timerOptions = <int?>[null, 15, 30, 60];
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        for (final t in _timerOptions)
                          ChoiceChip(
                            label: Text(t == null ? 'Off' : '${t}s'),
                            selected: _timerSeconds == t,
                            onSelected: (_) =>
                                setState(() => _timerSeconds = t),
                          ),
                      ],
                    ),
                    if (_timerSeconds != null)
                      Padding(
                        ...
                        child: Text(
                          'Running out just moves you on — it never marks the '
                          'question wrong.',
