...
class _ResultScreenState extends ConsumerState<ResultScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // Record each question's outcome locally. Answering a question that
      // was skipped in an earlier quiz overwrites its status, which is what
      // moves it out of the "skipped" slice and into correct/wrong.
      final outcomes = <String, AttemptOutcome>{};
      for (var i = 0; i < quiz.questions.length; i++) {
        final q = quiz.questions[i];
        final picked =
            i < quiz.selectedAnswers.length ? quiz.selectedAnswers[i] : -1;
        outcomes[q.id] = picked == -1
            ? AttemptOutcome.skipped
            : (picked == q.correctOptionIndex
                ? AttemptOutcome.correct
                : AttemptOutcome.wrong);
      }
      await AttemptStatsService().recordQuiz(outcomes);
    });
  }
